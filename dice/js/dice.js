...
(function (dice) {
    ...
    for (var i = 0; i < face_labels.length; ++i)
      materials.push(
        new THREE.MeshPhongMaterial(
          teal.copyto(this.material_options, {
  ...
  this.known_types = ["d4", "d6", "d8", "d10", "d12", "d20", "d100", "d10s"];
  ...
  this.create_d4 = function (color) {
    if (!this.d4_geometry)
      this.d4_geometry = this.create_d4_geometry(this.scale * 1.2);
    if (true) //!this.dice_material)
      this.dice_color = color || "#202020"
      this.label_color = getLuminance(this.dice_color) < 128 ? '#cccccc' : '#222222'
  ...
  this.create_d10 = function (color) {
  ...
  this.create_d12 = function (color) {
  ...
  this.create_d20 = function (color) {
  ...
  this.create_d100 = function (color) {
    if (!this.d10_geometry)
      this.d10_geometry = this.create_d10_geometry(this.scale * 0.9);
    if (true) //!this.dice_material)
    this.dice_color = color || "#202020"
  ...
  this.create_d10s = function (color) {
    if (!this.d10_geometry)
      this.d10_geometry = this.create_d10_geometry(this.scale * 0.9);
    if (true) //!this.dice_material)
    this.dice_color = color || "#202020"
    this.label_color = getLuminance(this.dice_color) < 128 ? '#cccccc' : '#222222'
      this.d10_material = new THREE.MeshFaceMaterial(
        this.create_dice_materials(
          this.standart_d20_dice_face_labels,
          this.scale / 2,
          1.5
  ...
  this.parse_notation = function (notation) {
    ...
    ret.colors = [];
    for (var i = 0; i < ret.set.length; i++) {
  ...
  function make_vector(size, notation, vector, boost) {
    var vec = make_random_vector(vector);
    var pos = {
      x: size.w * (vec.x > 0 ? -1 : 1) * 0.9,
      y: size.h * (vec.y > 0 ? -1 : 1) * 0.9,
      z: rnd() * 200 + 200,
    };
    ...
    var inertia = that.dice_inertia[notation];
    var angle = {
  ...
  this.dice_box.prototype.generate_vectors = function (
    ...
    for (var i = 0; i < notation.set.length; i++) {
        var color = getRandomColor();
        var v = make_vector({w: this.w, h: this.h}, notation.set[i], vector, boost)
        v.color = color;
        vectors.push(v);
  ...
  this.dice_box.prototype.create_dice = function (
    ...
    var dice = that["create_" + type](color);
